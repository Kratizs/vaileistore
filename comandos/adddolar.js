...
if(typeof chatfinal === 'undefined') {
  ...
  fs.writeFile("./verify/"+canal, data, (err) => {
    if (err) {
    return console.log(err);
    message.channel.send("Deu erro com alguma coisa :/");
  };
    console.log('1 Channel adicionado');
    message.channel.send("Opa, irei avisar ali quando o dólar sofrer alguma mudança")
  ...
  client.guilds.cache.get("822439547681243207").channels.cache.get("828450775054286889").send("O usuário: **<@" + message.author.id + ">** Adicionou 1 canal de dólar \nServer: **" + message.guild.name + "**(" + message.guild.id + ") \nNome do chat: **#" + message.channel.name + "**(" + message.channel.id + ")")
})
}
}};